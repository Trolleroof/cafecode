...
export class UserWorkspaceManager {
  ...
  static getUserWorkspacePath(userId) {
    ...
    if (!fs.existsSync(userDir)) {
      ...
      try {
        const welcomeFile = path.join(userDir, 'README.md');
        const welcomeContent = `# Welcome to Your Coding Workspace!

This is your personal coding environment. You can:
...
`;
  ...
  static getTerminalRoot(userId) {
    if (process.env.NODE_ENV === 'development' || process.env.LOCAL_TERMINAL_ROOT === '1') {
